...
function lockMobileHeroReviewHeight(){
  if(!heroReviews) return;
  heroReviews.style.minHeight='';
  if(!window.matchMedia('(max-width: 720px)').matches) return;

  const width=heroReviews.getBoundingClientRect().width;
  if(!width) return;

  const probe=heroReviews.cloneNode(false);
  probe.removeAttribute('id');
  probe.removeAttribute('aria-label');
  probe.setAttribute('aria-hidden','true');
  probe.style.cssText=`position:absolute!important;visibility:hidden!important;pointer-events:none!important;inset:auto!important;left:0!important;top:0!important;width:${width}px!important;margin:0!important;min-height:0!important;`;
  heroReviews.parentElement.appendChild(probe);

  let maxHeight=0;
  heroReviewData.forEach((review,index)=>{
    probe.dataset.slide=String(index+1);
    probe.innerHTML=review;
    maxHeight=Math.max(maxHeight,probe.scrollHeight);
  });

  probe.remove();
  heroReviews.style.minHeight=`${Math.ceil(maxHeight)}px`;
}
...
showHeroReview(0);
lockMobileHeroReviewHeight();
if(document.fonts?.ready) document.fonts.ready.then(lockMobileHeroReviewHeight);
let heroReviewResizeTimer;
window.addEventListener('resize',()=>{
  clearTimeout(heroReviewResizeTimer);
  heroReviewResizeTimer=setTimeout(lockMobileHeroReviewHeight,120);
});
